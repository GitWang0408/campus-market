...
public class HomeStudentController {
	 ...
	 * @return
	 */
	@RequestMapping(value="/task_publish_wanted",method=RequestMethod.POST)
	@ResponseBody
	public Result<Boolean> task_publishWanted(task_WantedGoods task_wantedGoods){
		CodeMsg validate = ValidateEntityUtil.validate(task_wantedGoods);
		if(validate.getCode() != CodeMsg.SUCCESS.getCode()){
			return Result.error(validate);
		}
		Student loginedStudent = (Student)SessionUtil.get(SessionConstant.SESSION_STUDENT_LOGIN_KEY);
		task_wantedGoods.setStudent(loginedStudent);
		if(task_wantedGoodsService.save(task_wantedGoods) == null){
			return Result.error(CodeMsg.HOME_STUDENT_PUBLISH_ERROR);
		}
		return Result.success(true);
	}
